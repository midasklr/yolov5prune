...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.st:
                ignore_bn_list = []
                mcv2_bn_list = []   # prune all bottlenet
                for k, m in model.named_modules():
                    if isinstance(m, Bottleneck):
                        if m.add and not k.startswith("model.2") and not k.startswith("model.4"):
                            # ignore_bn_list.append(k.rsplit(".", 2)[0] + ".cv1.bn")
                            mcv2_bn_list.append(k + '.cv1.bn')
                            mcv2_bn_list.append(k+".cv2.bn")
                            # ignore_bn_list.append(k + '.cv2.bn')
                    if isinstance(m, nn.BatchNorm2d):
                        if k in mcv2_bn_list:
                            m.weight.grad.data.add_(opt.sr*2 * torch.sign(m.weight.data))  # L1
                            m.bias.grad.data.add_(opt.sr*2 * torch.sign(m.bias.data))  # L1
                        elif k not in ignore_bn_list:
        ...
        for i, layer in model.named_modules():
            if isinstance(layer, nn.BatchNorm2d):
                if i in mcv2_bn_list: 
                    bnw = layer.state_dict()['weight']
                    bnb = layer.state_dict()['bias']
                    prune_all_w.append(bnw)
                    prune_all_b.append(bnb)                   
                elif i not in ignore_bn_list:
                    bnw = layer.state_dict()['weight']
                    bnb = layer.state_dict()['bias']
        ...
        size_p_list = [idx.data.shape[0] for idx in prune_all_w]

        bn_weights = torch.zeros(sum(size_list))
        bnb_weights = torch.zeros(sum(size_list))
        p_bn_w = torch.zeros(sum(size_p_list))
        p_bn_b = torch.zeros(sum(size_p_list))
        index = 0
        pindex = 0
        ...
        for idx, size in enumerate(size_p_list):
            p_bn_w[pindex:(pindex + size)] = prune_all_w[idx].data.abs().clone()
            p_bn_b[pindex:(pindex + size)] = prune_all_b[idx].data.abs().clone()
            pindex += size

        print("bn_weights:", torch.sort(bn_weights))
        print("bn_bias:", torch.sort(bnb_weights))
        print("prune m cv2 max weight : ", torch.max(p_bn_w))
        print("prune m cv2 max bias : ", torch.max(p_bn_b))
        ...
        tb_writer.add_histogram('bn_weights_cv2/hist', p_bn_w.numpy(), epoch, bins='doane')
        tb_writer.add_histogram('bn_bias_cv2/hist', p_bn_b.numpy(), epoch, bins='doane')
