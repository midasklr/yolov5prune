...
def test_prune(data,
    ...
    for i, layer in model.named_modules():
        ...
        if isinstance(layer, Bottleneck):
            if layer.add:
                ...
                bottlenet_cv2_list.append(i+".cv2.bn")
    ...
    pruned_yaml["backbone"] = [
        ...
        [-1, 3, C3PrunedNoRes, [128]],  # 2
        [-1, 1, Conv, [256, 3, 2]],  # 3-P3/8
        [-1, 9, C3PrunedNoRes, [256]],  # 4
        [-1, 1, Conv, [512, 3, 2]],  # 5-P4/16
        [-1, 9, C3PrunedNoRes, [512]],  # 6
    ...
    for bnname, bnlayer in model.named_modules():
        if isinstance(bnlayer, nn.BatchNorm2d):
            ...
            if bnname in bottlenet_cv2_list:
                print(bnname, "prune all bottlenet with shortcut\n")
                mask = torch.zeros(bnlayer.weight.data.size()).cuda()
    ...
    pruned_model = ModelPrunedNoRes(maskbndict=maskbndict, cfg=pruned_yaml, ch=3).cuda()
    ...
    for layername, layer in model.named_modules():
        prune_map[layername] = layer

    for layername, pruned_layer in pruned_model.named_modules():
        layer = prune_map[layername]
        if isinstance(pruned_layer, nn.Conv2d) and not layername.startswith("model.24"):
    ...
    model = pruned_model
    torch.save({"model":model}, "pruned_model_nores.pt")
    model.cuda().eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/kong/yolov5/runs/train/exp152/weights/last.pt',
                        help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='data/mini.yaml', help='*.data path')
    parser.add_argument('--percent', type=float, default=0.85, help='prune percentage')
